...
.slider-toast-brief   { border-left: 3px solid #60a5fa; }
.slider-toast-standard{ border-left: 3px solid #4ade80; }
.slider-toast-thorough{ border-left: 3px solid #fbbf24; }
...
.sp-outer {
    ...
    background: linear-gradient(145deg,
        ...
        rgba(219,239,255,0.70) 55%,
        rgba(209,231,255,0.78) 100%);
    ...
    box-shadow:
        0 10px 40px rgba(15,76,117,0.11),
...
.sp-outer::before {
    ...
    background: radial-gradient(circle, rgba(27,108,168,0.10) 0%, transparent 65%);
    pointer-events: none;
}
.sp-outer::after {
    content: '';
...
.sp-icon-box {
    ...
    background: linear-gradient(135deg, #0c3d63 0%, #1b6ca8 100%);
    display: flex; align-items: center; justify-content: center; flex-shrink: 0;
    box-shadow: 0 6px 18px rgba(15,76,117,0.38);
}
.sp-head-title {
    font-size: 1.08rem; font-weight: 800; color: #0a2a45;
    letter-spacing: -0.015em;
}
.sp-head-sub  { font-size: 0.76rem; color: #5a85b0; margin-top: 3px; }
.sp-head-badge {
    margin-left: auto; flex-shrink: 0;
    background: linear-gradient(90deg, #dbeafe, #e0f2fe);
    border: 1px solid #93c5fd; border-radius: 20px;
    padding: 4px 13px; font-size: 0.7rem; font-weight: 700;
    color: #1d4ed8; letter-spacing: 0.05em;
}
.sp-hint-text {
    padding-top: 0.85rem; font-size: 0.78rem;
    color: #6b90b8; font-style: italic;
}
.sp-count-pill {
    display: inline-flex; align-items: center; gap: 7px;
    background: linear-gradient(90deg,
        rgba(16,185,129,0.13) 0%, rgba(5,150,105,0.07) 100%);
...
.sp-divider {
    height: 1px;
    background: linear-gradient(90deg,
        transparent 0%, rgba(15,76,117,0.14) 20%,
        rgba(15,76,117,0.22) 50%,
        rgba(15,76,117,0.14) 80%, transparent 100%);
    margin: 0 0 1.2rem;
}
.sp-select-label {
    display: flex; align-items: center; gap: 8px;
    font-size: 0.75rem; font-weight: 700; color: #1e4a7a;
...
.sp-selected-card {
    ...
    background: linear-gradient(135deg,
        rgba(15,76,117,0.07) 0%, rgba(27,108,168,0.04) 100%);
    border: 1.5px solid rgba(15,76,117,0.22);
    border-radius: 14px; padding: 13px 16px;
    margin-top: 11px;
    animation: fadeSlideIn 0.25s ease;
    box-shadow: 0 2px 10px rgba(15,76,117,0.07);
}
.sp-selected-icon {
    ...
    box-shadow: 0 4px 12px rgba(15,76,117,0.32);
}
.sp-selected-name {
    font-size: 0.9rem; color: #0a2a45; font-weight: 700;
    white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
    max-width: 480px; line-height: 1.3;
}
.sp-selected-sub {
    font-size: 0.72rem; color: #5a85b0; margin-top: 3px;
...
.sp-ext-badge {
    ...
    color: #0f4c75; letter-spacing: 0.05em;
}
.sp-empty {
    text-align: center; padding: 1.8rem 1rem;
    border: 1.5px dashed rgba(15,76,117,0.16);
    border-radius: 16px; margin: 0.5rem 0;
    background: rgba(255,255,255,0.35);
}
.sp-empty-icon { font-size: 2.2rem; margin-bottom: 0.5rem; }
.sp-empty-text { font-size: 0.88rem; color: #6b90b8; font-weight: 600; }
.sp-empty-sub  { font-size: 0.74rem; color: #9ab8cc; margin-top: 5px; }
...
def generate_pdf(summary_html, fname, ext2, detail_level):
    try:
        ...
        pdf.cell(
            0, 6,
            f"File: {fname}   |   Format: {ext2}   |   Detail: {detail_level}",
            new_x=XPos.LMARGIN, new_y=YPos.NEXT
        )
...
# ── FIX: initialize detail_level in session state BEFORE the slider ──
...
with st.sidebar:
    ...
    # ── FIX: no value= param; key alone manages state ──
    ...
    _toast_cfg = {
        "Brief":    ("slider-toast-brief",    "💨", "Set to Brief — concise overview"),
        "Standard": ("slider-toast-standard", "✅", "Set to Standard — balanced summary"),
        "Thorough": ("slider-toast-thorough", "🔍", "Set to Thorough — in-depth analysis"),
    }
    _cls, _icon, _msg = _toast_cfg[detail_level]
    st.markdown(
        f'<div class="slider-toast {_cls}"><span style="font-size:0.85rem;flex-shrink:0">{_icon}</span>{_msg}</div>',
        unsafe_allow_html=True,
    )
...
if uploaded_files:
    if st.button("✦  Summarize Document", key="upload_summarize_btn"):
        files = [("files", (file.name, file, file.type)) for file in uploaded_files]
        with st.spinner("Analyzing your document with AI…"):
            response = requests.post(
                f"{BASE_URL}/upload-multiple",
                files=files,
                params={"user_id": USER_ID, "detail_level": detail_level},
            )
        if response.status_code == 200:
            ...
            st.session_state.summary_detail = detail_level
...
# SHAREPOINT SECTION
# ══════════════════════════════════════════════════════════════════════
st.markdown('<div class="sp-outer">', unsafe_allow_html=True)
...
<div class="sp-header-row">
    ...
    <div>
        ...
        <div class="sp-head-sub">Connect to OneDrive · Pick a document · Summarize instantly</div>
    </div>
    <div class="sp-head-badge">☁ CLOUD</div>
</div>
""", unsafe_allow_html=True)
...
with hint_col:
    st.markdown(
        '<div class="sp-hint-text" style="padding-top:0.85rem;">Fetch the latest files from your SharePoint / OneDrive</div>',
        unsafe_allow_html=True,
    )
    ...
    with st.spinner("Connecting to SharePoint…"):
        try:
            ...
            if res.status_code == 200:
                ...
                st.session_state.synced_count = count
...
if st.session_state.show_files:
    ...
    pill_label = f"{total} file{'s' if total != 1 else ''} available in SharePoint"
    st.markdown(
        f'<div class="sp-count-pill"><span class="sp-count-dot"></span>{pill_label}</div>',
        unsafe_allow_html=True,
    )
    ...
    if not synced_files:
        ...
        <div class="sp-empty">
            ...
            <div class="sp-empty-text">No files found in SharePoint</div>
            <div class="sp-empty-sub">Make sure your OneDrive sync is active and retry</div>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown('<div class="sp-divider"></div>', unsafe_allow_html=True)
        st.markdown("""
        <div class="sp-select-label">
            <span class="sp-select-dot"></span>
        ...
        if st.session_state.selected_sp_file:
            ...
            display_name    = clean_name(sel_file)
            icon, ext_badge = get_ext_icon(display_name)

            st.markdown(f"""
            <div class="sp-selected-card">
                <div class="sp-selected-icon">{icon}</div>
                <div style="overflow:hidden;flex:1;">
                    <div class="sp-selected-name">{short_name(display_name, 62)}</div>
                    <div class="sp-selected-sub">
                        <span class="sp-ext-badge">{ext_badge}</span>
                        Ready to summarize
                    </div>
                </div>
            </div>
...
st.markdown("</div>", unsafe_allow_html=True)  # close sp-outer
...
if "selected_file" in st.session_state:
    ...
    with st.spinner("Analyzing document with AI…"):
        try:
            res = requests.post(
            f"{BASE_URL}/upload",
            params={"user_id": USER_ID, "detail_level": detail_level},  # ✅ FIX
            data={"file_name": file},
)
            if res.status_code == 200:
                data = res.json()
